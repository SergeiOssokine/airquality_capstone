...
echo "Triggering the analysis flow"
# Get account ID
account_id=$(
...
deployment_id=$(curl -s -X GET "${base_url}/accounts/${account_id}/workspaces/${workspace_id}/deployments/name/airquality-analysis-flow/airquality-analysis" -H "$header_auth" -H "$header_type" | jq -r '.id')
